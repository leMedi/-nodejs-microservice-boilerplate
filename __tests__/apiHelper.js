...
export async function apiHelper() {
  ...
  return {
    ...
    todo: {
      list: params => client.get(`/todos`).then(assertStatus(200)),
      get: id => client.get(`/todos/${id}`).then(assertStatus(200)),
      create: data => client.post('/todos', data).then(assertStatus(201))
    }
  }
}
